...
if __name__ == '__main__':
    ...
    # CONV-RELU-POOL 3
    conv4 = Conv2D(32, (3, 3), activation="relu")(batch_normal3)
    ...
    x_batch = []
    for i in range(32):
        x_output = Lambda(lambda x: x[:, :, :, i])(batch_normal4)
        x_batch.append(x_output)

    outputs = []
    shared_gru = GRU(15, return_sequences=True)
    shared_gru2 = GRU(20, return_sequences=False)
    for i in range(32):
        output = shared_gru(x_batch[i])
        output_bn = BatchNormalization(axis=-1)(output)

        output_2 = shared_gru2(output_bn)
        output_bn2 = BatchNormalization(axis=-1)(output_2)

        outputs.append(output_bn2)
    ...
    batch_normal_out = BatchNormalization(axis=-1)(gru_output)
