...
def create_model(optimizer='SGD', init='glorot_uniform'):
    ...
    # CONV-RELU-POOL 1
    conv1 = Conv2D(16, (5, 5), activation="relu", kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01))(model_input)
    pool1 = MaxPooling2D(pool_size=(3, 3), strides=(1, 2), padding='same')(conv1)
    batch_normal1 = BatchNormalization(axis=-1)(pool1)
    # CONV-RELU-POOL 2
    conv2 = Conv2D(32, (3, 3), activation="relu", kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01))(batch_normal1)
    pool2 = MaxPooling2D(pool_size=(3, 3), strides=(1, 2), padding='same')(conv2)
    batch_normal2 = BatchNormalization(axis=-1)(pool2)
    # CONV-RELU-POOL 3
    conv3 = Conv2D(32, (3, 3), activation="relu", kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01))(batch_normal2)
    pool3 = MaxPooling2D(pool_size=(3, 3), strides=(1, 2), padding='same')(conv3)
    batch_normal3 = BatchNormalization(axis=-1)(pool3)
    # CONV-RELU-POOL 3
    conv4 = Conv2D(32, (3, 3), activation="relu", kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01))(batch_normal3)
    ...
    # output-layer
    model_output = Dense(10, activation="softmax", kernel_initializer=init,
                         kernel_regularizer=regularizers.l2(0.01))(dropout1)
    model = Model(inputs=model_input, outputs=model_output)
    #model.compile(loss=categorical_crossentropy, optimizer=keras.optimizers.SGD(lr=2e-3), metrics=['accuracy'])
...
if __name__ == '__main__':
    ...
    # 定义网格搜索参数
    batch_size = [16, 32, 48, 64, 100, 128]  # 128
    epochs = [30, 50, 70, 100]  # 100
    init = ['glorot_uniform', 'normal', 'uniform']
    optimizer = ['SGD', 'RMSprop', 'Adagrad', 'Adadelta', 'Adam', 'Adamax', 'Nadam']
    param_grid = dict(batch_size=batch_size, nb_epoch=epochs, optimizer=optimizer, init=init)

    # learn_rate = [0.001,0.003, 0.01, 0.1, 0.2, 0.3]
    # momentum = [0.0, 0.2, 0.4, 0.6, 0.8, 0.9]
    # param_grid = dict(learn_rate=learn_rate, momentum=momentum)
    # # # 模型编译
    grid = GridSearchCV(estimator=model, param_grid=param_grid, n_jobs=1)
    grid_result = grid.fit(val_wavs, val_labels)
    #输出调参结果
    ...
    #
    # # 模型训练,checkpoint
    # filepath = "./model/cnn4no_pool_single_gru1fc0shared-{epoch:02d}-{val_acc:.2f}.hdf5"
    # # 中途训练效果提升, 则将文件保存, 每提升一次, 保存一次
    # checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True,
    #                              mode='max')
    #
    # reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=10, mode='auto')
    # hist = model.fit(wavs, labels, batch_size=64, epochs=70, validation_data=(val_wavs, val_labels),
    ...
    #history.end_draw()
    # loss: 0.1586 - acc: 0.9519 - val_loss: 0.2516 - val_acc: 0.9245
