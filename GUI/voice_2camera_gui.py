...
def predict(audiopath):
    ...
    if np.max(predict) < 0.8:
        ...
        windows.update()
        global cnt
        if labsIndName[predict[0]] == "start":
            cnt["start"] +=1
            if cnt["start"] ==1:
                os.system(' gnome-terminal --geometry 20x10+1+1 -x bash -c "source ~/catkin_ws/devel/setup.bash; '
                      'roslaunch aubo_i5l0_moveit_config demo.launch;read" ')
            else:
                pass
        elif labsIndName[predict[0]] == "reset":
            os.system(' gnome-terminal --geometry 20x10+1+1 -x bash -c "source ~/catkin_ws/devel/setup.bash; '
                      'rosrun aubo_demo reset;read" ')

        elif labsIndName[predict[0]] == "left":
            cnt["left"] +=1
            if cnt["left"] == 1 or cnt["left"] == 2:
                ...
                pass
        elif labsIndName[predict[0]] == "right":
            cnt["right"] +=1
            if cnt["right"] == 1:
                os.system(' gnome-terminal --geometry 20x10+1+1 -x bash -c "source ~/catkin_ws/devel/setup.bash; '
            ...
            if cnt["up"] ==1:
                ...
                pass
        elif labsIndName[predict[0]] == "down":
            cnt["down"] +=1
            if cnt["down"] ==1:
                os.system(' gnome-terminal --geometry 20x10+1+1 -x bash -c "source ~/catkin_ws/devel/setup.bash; '
                ...
                pass
        elif labsIndName[predict[0]] == "feed":
            cnt["left"] = 0
            cnt["right"] = 0
            cnt["up"] = 0
            cnt["down"] = 0
        ...
        elif labsIndName[predict[0]] == "back":
            cnt["left"] =0
            cnt["right"] = 0
            cnt["up"] = 0
            cnt["down"] = 0
        ...
        elif labsIndName[predict[0]] == "stop":
...
def start_capture2():
    ...
    def cc2():
        ...
        while allowCapture2:
            ...
            obr2 = image_file2  # 这条指令必须有，否则画面会闪烁
    t2=threading.Thread(target=cc2)
    t2.start()
def stop_capture2():
    global allowCapture2
    allowCapture2 = False
